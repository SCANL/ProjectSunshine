    ...
    def __init__(self):
        ...
        if path.exists(results_file):
            ...
            self.results_file.write(
                '"FilePath","FileType","Identifier","IdentifierType","LineNumber","IssueID", "IssueAdditionalDetail", "IssueCategory","IssueDetail", "AnalysisDateTime"\n')
    ...
    def save_issues(self, issues):
        for issue in issues:
            self.results_file.write('"%s","%s","%s","%s","%s","%s","%s","%s","%s","%s"\n' % (
                ...
                issue.id, issue.additional_details, issue.category, issue.details,
