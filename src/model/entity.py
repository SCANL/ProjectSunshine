...
class Entity:
    ...
    def is_using_testing_package(self, source: etree._ElementTree) -> None:
        ...
        if self.language == enum.LanguageType.Java:
            all_imports = source.xpath(
                '//src:import/src:name', namespaces={'src': 'http://www.srcML.org/srcML/src'})
            for i in range(len(all_imports)+1):
                partial_name = source.xpath(
                    '//src:import['+str(i)+']/src:name//text()', namespaces={'src': 'http://www.srcML.org/srcML/src'})
        ...
        if self.language == enum.LanguageType.CSharp:
            all_imports = source.xpath(
                '//src:using/src:name', namespaces={'src': 'http://www.srcML.org/srcML/src'})
            for i in range(len(all_imports)+1):
                partial_name = source.xpath(
                    '//src:using['+str(i)+']/src:name//text()', namespaces={'src': 'http://www.srcML.org/srcML/src'})
                if ''.join(partial_name) in cast(List[str], testing_packages):
                    self.file_type = enum.FileType.Test
                    break
    ...
    def construct_hierarchy(self):
        ...
        self.language = LanguageType.get_type(tree.xpath(
            '//src:unit/@language', namespaces={'src': 'http://www.srcML.org/srcML/src'})[0])
    ## ----------------------------------------------------------------------------------------------------------------##
        self.is_using_testing_package(tree)
    ## ----------------------------------------------------------------------------------------------------------------##
        class_list = tree.xpath(
            '//src:class', namespaces={'src': 'http://www.srcML.org/srcML/src'})
        for class_item in class_list:
            if len(class_item.xpath('./src:name', namespaces={'src': 'http://www.srcML.org/srcML/src'})) == 0:
                continue
            class_name = class_item.xpath(
                './src:name', namespaces={'src': 'http://www.srcML.org/srcML/src'})[0]
            if class_name.text is None:
                class_name = class_item.xpath(
                    './src:name/src:name', namespaces={'src': 'http://www.srcML.org/srcML/src'})[0]
            model_class = Class(class_name.text, class_item)
            class_comment = tree.xpath(
                '//src:class[src:name=\''+model_class.name+'\']/preceding-sibling::*[1][self::src:comment]', namespaces={'src': 'http://www.srcML.org/srcML/src'})
    ...
    ## ----------------------------------------------------------------------------------------------------------------##
            attribute_list = class_item.xpath(
                '*/src:decl_stmt/src:decl', namespaces={'src': 'http://www.srcML.org/srcML/src'})
            for i, attribute_item in enumerate(attribute_list):
                try:
                    attribute_specifier = attribute_item.xpath(
                        './src:type/src:specifier', namespaces={'src': 'http://www.srcML.org/srcML/src'})
                    ...
                    attribute_type = attribute_item.xpath(
                        './src:type/src:name', namespaces={'src': 'http://www.srcML.org/srcML/src'})
                    ...
                    if len(attribute_type) != 0:
                        ...
                        if attribute_type.text == None:
                            attribute_type = attribute_item.xpath(
                                './src:type/src:name/src:name', namespaces={'src': 'http://www.srcML.org/srcML/src'})[0]
                            if attribute_type.text == None:
                                attribute_type = attribute_item.xpath(
                                    './src:type/src:name/src:name/src:name', namespaces={'src': 'http://www.srcML.org/srcML/src'})[0]
                            attribute_array = attribute_item.xpath(
                                './src:type/src:name/src:index', namespaces={'src': 'http://www.srcML.org/srcML/src'})
                        ...
                        attribute_comment = class_item.xpath('//src:decl_stmt[src:decl/src:type/src:name=\''+attribute_type.text+'\' and src:decl/src:name=\'' +
                                                             attribute_name.text+'\']/preceding-sibling::*[1][self::src:comment]', namespaces={'src': 'http://www.srcML.org/srcML/src'})
    ...
    ## ----------------------------------------------------------------------------------------------------------------##
            ...
            method_list = class_item.xpath(
                '*/src:function', namespaces={'src': 'http://www.srcML.org/srcML/src'})
            for method_item in method_list:
                try:
                    ...
                    method_annotation = method_item.xpath(
                        './src:annotation/src:name', namespaces={'src': 'http://www.srcML.org/srcML/src'})
                    method_annotation = [''.join(x.text)
                                         for x in method_annotation]
                    method_return_type = method_item.xpath(
                        './src:type/src:name', namespaces={'src': 'http://www.srcML.org/srcML/src'})[0]
        ...
        ## ----------------------------------------------------------------------------------------------------------------##
                    parameter_list = method_item.xpath(
                        '*/src:parameter/src:decl', namespaces={'src': 'http://www.srcML.org/srcML/src'})
                    for parameter_item in parameter_list:
                        ...
                        if len(parameter_type) != 0:
                            ...
                            model_parameter = Parameter(
                                parameter_type.text, parameter_name.text, parameter_type_array, parameter_type_generic, parameter_item)
                    ...
                    for num, parameter in enumerate(model_method.parameters, start=1):
                        if parameter.type_is_generic:
                            xpath_strings.append("(src:parameter_list/src:parameter["+str(num)+"]/src:decl/src:name='"+parameter.name+"' and src:parameter_list/src:parameter["+str(
                                num)+"]/src:decl/src:type/src:name/src:name='"+parameter.type+"')")
                        else:
                            xpath_strings.append("(src:parameter_list/src:parameter["+str(num)+"]/src:decl/src:name='"+parameter.name +
                                                 "' and src:parameter_list/src:parameter["+str(num)+"]/src:decl/src:type/src:name='"+parameter.type+"')")
                    xpath_string = "//src:function[src:name='" + \
                        model_method.name+"'"
        ...
        ## ----------------------------------------------------------------------------------------------------------------##
                    ...
                    for variable_item in variable_list:
                        ...
                        if len(variable_type) != 0:
                            ...
                            variable_comment = method_item.xpath('//src:decl_stmt[src:decl/src:type/src:name=\'' + variable_type.text + '\' and src:decl/src:name=\'' +
                                                                 variable_name.text + '\']/preceding-sibling::*[1][self::src:comment]', namespaces={'src': 'http://www.srcML.org/srcML/src'})
