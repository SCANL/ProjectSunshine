    ...
    def __process_identifier(self, identifier):
        ...
        try:
            if is_singular(self.__project, identifier.name_terms[-1]):
                if identifier.type in get_collection_types(self.__project, self.entity.language) or identifier.is_array == True:
                    issue = Issue(self, identifier)
                    issue.additional_details = 'Last term: \'%s\' Data type: \'%s%s\'' % (
