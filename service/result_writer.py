    ...
    def __init__(self):
        ...
        if path.exists(results_file):
            ...
            self.results_file.write(
                '"FilePath","Identifier","IdentifierType","IssueID", "IssueAdditionalDetail", "IssueCategory","IssueDetail", "AnalysisDateTime"\n')
            self.results_file.flush()

    def save_issues(self, issues):
        for issue in issues:
            self.results_file.write('"%s","%s","%s","%s","%s","%s","%s","%s"\n' % (
                issue.file_path, issue.identifier, issue.identifier_type.name, issue.id, issue.additional_details,
